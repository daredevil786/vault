...
export default Controller.extend({
  ...
  disableEngine: task(function*(engine) {
    ...
    try {
      ...
      this.get('flashMessages').success(`The ${engineType} engine at ${path} has been disabled.`);
    } catch (err) {
      this.get('flashMessages').danger(
        `There was an error disabling the ${engineType} engine at ${path}: ${err.errors.join(' ')}.`
      );
    }
  }).drop(),
});
